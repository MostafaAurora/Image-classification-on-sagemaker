...
# Setting up some basic configs to enable logging
...
def net():
    ...
    for param in model.parameters():
        param.requires_grad = False # Freezeing all Convolutional layers
    
    num_features = model.fc.in_features
    model.fc = nn.Sequential( nn.Linear( num_features, 256), # Adding two fully connected layers
                             nn.ReLU(inplace = True),
                             nn.Linear(256, 133), # matching the output nodes to the number os classes/categories i.e dog breeds which is a 133 in this case
                             nn.ReLU(inplace = True) 
                            )
    return model

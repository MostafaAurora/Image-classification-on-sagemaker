...
def net():
    ...
    model.fc = nn.Sequential( nn.Linear( num_features, 256), # Adding our own fully connected layers
                             ...
                             nn.Linear(256, 133),
                             nn.ReLU(inplace = True) # Matching the number output nodes to the number of classes/categories i.e dog breeds which is a 133 in this case
                            )
    return model



# Overriding the model function to load our model 
...
# Overriding the default input function

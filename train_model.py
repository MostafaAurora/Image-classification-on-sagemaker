...
def test(model, test_loader, criterion, device, epoch_no, hook):
    ...
    model.eval()
    hook.set_mode(smd.modes.EVAL) # Setting debugger hook mode to EVAL
    running_loss = 0
    running_corrects = 0
    with torch.no_grad():
        for inputs, labels in test_loader:
            ...
            running_corrects += pred.eq(labels.view_as(pred)).sum().item() # Calculating the running corrects
...
def train(model, train_loader, criterion, optimizer, device, epoch_no, hook):
    ...
    model.train()
    hook.set_mode(smd.modes.TRAIN) # Setting debugger hook mode to TRAIN
    running_loss = 0
    running_corrects = 0
    running_samples = 0
    for inputs, labels in train_loader:
        ...
        loss = criterion(outputs, labels)
        pred = outputs.argmax(dim=1,  keepdim=True)
        running_loss += loss.item() * inputs.size(0) # Calculating running loss
        running_corrects += pred.eq(labels.view_as(pred)).sum().item() # Calculating running corrects
        running_samples += len(inputs) # keeping count of running samples
        loss.backward()
        optimizer.step()
...
def net():
    model = models.resnet50(pretrained = True) # Using a pre trained RESNET50 model with 50 layers
    
    for param in model.parameters():
        param.requires_grad = False # Freezing all the Convolutional layers
    
    num_features = model.fc.in_features
    model.fc = nn.Sequential( nn.Linear( num_features, 256), # Adding two fully connected layers
                             nn.ReLU(inplace = True),
                             nn.Linear(256, 133), # Matching the number output nodes to the number of classes/categories i.e dog breeds which is a 133 in this case
                             nn.ReLU(inplace = True)
                            )
    return model
...
def main(args):
    ...
    # Using AdamW as it usually (generalizes better / delivers better results) than the original adam due to decoupling weight decay from learning rate 
    optimizer = optim.AdamW(model.fc.parameters(), lr=args.lr, eps= args.eps, weight_decay = args.weight_decay)

    # Adding in the epoch to train and test/validate for the same epoch simultaneously
...
if __name__=='__main__':
    ...
    # Using sagemaker OS Environ's channels to locate training data, model directory and output directory to save in S3 bucket
